...
void UserInterface::remove()
{
    ...
    if (input!="")
    {
        ...
        if (valid && id>0) {
            ...
            m_currentRegion->regionRemover(m_currentRegion->findRegionid(id));
        }
        else
        {
...
int UserInterface::getRegionid(unsigned int id)
{
    bool isFound= false;

    while(isFound!= true)
    {
        for (int i = 0; i < m_currentRegion->getSubRegionCount(); ++i)
        {
