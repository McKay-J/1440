...
bool Point::isEquivalentTo(const Point &otherPoint) const
{
    ...
    return edge.getLength() < m_minDistance; //TODO $$ This was set to Less than
}
...
void Point::initialize(const std::string &pointStr)
{
    ...
    if (split(pointStr, ',', values, 3))
    {
        ...
        m_z = convertStringToDouble(values[2], &m_valid); //TODO $$ m_z was set to m_y
        if(m_valid)
        {
            checkForInfinity();
        }
...
{
    m_valid = (m_x != INFINITY && m_y != INFINITY && m_z != INFINITY); //TODO $$Changed m_z to not equal to
}
